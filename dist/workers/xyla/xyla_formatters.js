...
(function () {
    ...
    xyla.formatters = {
         ...
         */
        affix: function (prefix, suffix, separator) {
            if (separator === undefined) {
                separator = '';
            }
            return function (value) { return xyla.formatters.empty(true, false)(value) ? '' : [prefix, value, suffix].filter(function (s) { return s !== undefined && s !== null; }).join(separator); };
        },
        /**
         ...
         */
        empty: function (emptyValue, fullValue) {
         ...
         */
        string: function () {
            return function (value) { return '' + xyla.formatters.empty()(value); };
        },
        /**
        ...
        date: function (format) {
            ...
            return function (value) { return xyla.formatters.empty(true, false)(value) ? '' : Highcharts.dateFormat(format, new Date(value).getTime()); };
        },
        /**
         ...
         */
        number: function (precision, decimal, separator) {
            if (separator === undefined) {
                separator = ',';
            }
            return function (value) { return xyla.formatters.empty(true, false)(value) ? '' : Highcharts.numberFormat(value, precision, decimal, separator); };
        },
        /**
         ...
         */
        dateAdd: function (offset, datePart) {
            if (!offset) {
                offset = 0;
            }
            offset = parseFloat(offset);
            if (isNaN(offset)) {
                return xyla.tools.presentError(new Error('A number is required for dateAdd formatter offset.'));
            }
            if (!datePart) {
                datePart = 'days';
            }
            if (['month', 'months', 'mos', 'mo'].indexOf(datePart) !== -1) {
                return function (value) {
                    if (xyla.formatters.empty(true, false)(value)) {
                        return '';
                    }
            ...
            return function (value) {
                ...
                var valueMillis = new Date(value).getTime();
         ...
         */
        dateRange: function (interval, datePart, format, separator) {
            if (!interval) {
                interval = 0;
            }
            if (!datePart) {
            ...
            return function (value) {
                ...
                var startDate = xyla.formatters.dateAdd(0)(value);
                var endDate = xyla.formatters.dateAdd(interval, datePart)(value);
                var formattedStartDate = xyla.formatters.date(format)(startDate);
                var formattedEndDate = xyla.formatters.date(format)(endDate);
        ...
        format: function (format) {
            var splitFormat = xyla.tools.extractQuoted(format);
            var formatParts = splitFormat.shift().split(/{(.*?)}/);
            return function (value, context) {
                var targets = Object.assign({ value: value }, context);
                var quoteSubstitutions = splitFormat.slice();
                return formatParts.map(function (part, index) {
                    if (index % 2 === 0) {
                        return xyla.tools.substituteQuoted([part, quoteSubstitutions]);
                    }
                    var pipes = part.split('|');
                    var targetName = xyla.tools.substituteQuoted([pipes.shift(), quoteSubstitutions]);
                    var target = targets[targetName];
                    pipes.forEach(function (pipe) {
                        var parameters = pipe.split(':').map(function (s) { return xyla.tools.substituteQuoted([s, quoteSubstitutions]); });
                        var pipeName = parameters.shift();
                        if (Object.keys(xyla.formatters).indexOf(pipeName) === -1) {
                            return;
                        }
...
//# sourceMappingURL=xyla_formatters.js.map