...
(function () {
    ...
    xyla.bi.mode.dataSet = function (queryName) {
        var dataSet = datasets.filter(function (dataset) { return dataset.queryName === queryName; }).pop();
        if (!dataSet) {
            return xyla.tools.presentError(new Error(arguments.callee.name + " dataset not found. " + queryName));
        }
        return dataSet;
    };
    /**
    ...
    xyla.bi.mode.groupNameForRow = function (row, groupByColumns) {
        return JSON.stringify(groupByColumns.map(function (column) { return row[column]; }));
    };
    /**
    ...
    xyla.bi.mode.groupedRows = function (dataSet, groupByColumns) {
        ...
        dataSet.content.forEach(function (row) {
            var groupName = _this.groupNameForRow(row, groupByColumns);
    ...
    xyla.bi.mode.setNeedsFilterUpdate = function () {
        ...
        var self = this;
        setTimeout(function () { return $(document).ready(function () {
    ...
    xyla.bi.mode.filterDateString = function (dateStringQueryName, dateStringColumn, dateQueryName, dateColumn) {
        var dataset = xyla.bi.mode.dataSet(dateStringQueryName);
        var dateDataset = xyla.bi.mode.dataSet(dateQueryName);
        if (!dataset || !dateDataset) {
            return;
        }
        dataset.content = dateDataset.content.map(function (dateRow) {
            var row = {};
    ...
    xyla.bi.mode.recalculateRatioColumnOnFilterChange = function (queryName, numeratorColumn, denominatorColumn, ratioColumn, groupByColumns, ratioColumnWillBeSummed) {
        ...
        var numeratorTotals = {};
        var denominatorTotals = {};
        var groupCounts = {};
        dataset.content.forEach(function (row) {
            var group = groupForRow(row);
    ...
    xyla.bi.mode.recalculateCumulativeSumColumnOnFilterChange = function (queryName, termColumn, seriesColumn, cumulativeSumColumn, groupByColumns) {
        ...
        var _loop_1 = function () {
            ...
            group.sort(function (a, b) { return (a[seriesColumn] < b[seriesColumn]) ? -1 : (a[seriesColumn] > b[seriesColumn]) ? 1 : 0; });
            var sum = 0;
...
//# sourceMappingURL=xyla_bi.js.map